...
router.post("/", isAuthenticated, async (req, res) => {
  try {
    const { category, severity, contentType, contentId, description } = req.body;
  ...
  try {
    ...
    const reports = await Report.find(filter)
      ...
      .sort({ 
        ...
        category: category === "violence" ? -1 : category === "hateSpeech" ? -2 : category === "harassment" ? -3 : 1,
...
router.get("/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    if (!req.user.isAdmin && report.reporterId._id.toString() !== req.user._id.toString()) {
...
router.get("/user/submitted", isAuthenticated, async (req, res) => {
  try {
