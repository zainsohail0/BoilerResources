  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!finalUserId) {
          throw new Error(
            "User ID could not be determined. Please log in again."
          );
        }
        ...
        const groupsRes = await fetch(
          `${API_URL}/api/groups/class/${classId}`,
          {
            headers,
            credentials: "include",
          }
        ...
        for (const group of groupsData) {
          try {
            const statusRes = await fetch(
              `${API_URL}/api/groups/${group._id}/user-status`,
              {
            ...
            if (statusRes.ok) {
              ...
              console.error(
                `Failed to fetch status for group ${group._id}. Status: ${statusRes.status}`
              );
              ...
              statusesObj[group._id] = {
                ...
                status:
                  group.adminId === finalUserId
                    ? "admin"
                    : group.members?.includes(finalUserId)
                    ? "member"
                    : "none",
              };
            }
            ...
            console.error(
              `Error fetching status for group ${group._id}:`,
              error
            );
            statusesObj[group._id] = {
              // Fallback on error
  ...
  const handleJoinGroup = async (groupId, isPrivate) => {
    try {
      ...
      if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        result = {
          message:
            text || (response.ok ? "Action successful" : "An error occurred"),
        };
      }
      ...
      setSuccessMessage(
        result.message ||
          `You've ${
            isPrivate ? "requested to join" : "joined"
          ...
          <div>
            ...
            {studyGroups.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {studyGroups.map((group) => {
                  const userStatus = userStatuses[group._id] || {
                    // Default if status hasn't loaded yet
                  ...
                  const showPendingForAdmin =
                    adminRequestState[group._id] === "pending";
                  // --- END: Modified logic ---
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="flex items-center mt-1 mb-2 flex-wrap">
                          ...
                          {/* Added flex-wrap */}
                          <span
                            className={`px-2 py-1 rounded text-xs mr-2 mb-1 ${
                              group.isPrivate
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 text-center">
                ...
                {/* Centered text */}
                <p className="text-gray-600 dark:text-gray-400">
                  No study groups available for this class yet. Be the first to
                  create one!
                </p>
              </div>
            )}
