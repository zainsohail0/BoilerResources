...
const AddClass = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const subjectSet = new Set(coursesData.map(course => course.subject || getSubjectFromCode(course.courseCode)));
        // Use standard credit options 1-5 instead of extracting from data
        const creditOptions = [1, 2, 3, 4, 5];
        const typeSet = new Set(coursesData.map(course => course.type || "General"));
    ...
    const savedSubjects = JSON.parse(localStorage.getItem('filterSubjects')) || [];
    const savedCredits = JSON.parse(localStorage.getItem('filterCredits')) || [];
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() !== "") {
      results = results.filter(
        (classItem) =>
          (classItem.courseCode && classItem.courseCode.toLowerCase().includes(searchTerm.toLowerCase())) ||
          (classItem.title && classItem.title.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
    ...
    if (selectedSubjects.length > 0) {
      results = results.filter(classItem => {
        const subject = classItem.subject || getSubjectFromCode(classItem.courseCode);
    ...
    if (selectedCredits.length > 0) {
      results = results.filter(classItem => 
        classItem.creditHours && selectedCredits.includes(classItem.creditHours)
      );
    }
    ...
    if (hasActiveFilters() && results.length === 0 && searchResults.length > 0) {
      setInvalidFilterMessage("No classes match your filter criteria. Try adjusting your filters.");
    }
    
    setFilteredResults(results);
  }, [searchTerm, selectedSubjects, selectedCredits, selectedClassTypes, searchResults]);
  ...
  const fetchUserClasses = async (id) => {
    try {
      ...
      const credits = classes.reduce((sum, classItem) => sum + (classItem.creditHours || 0), 0);
  ...
  const hasActiveFilters = () => {
    return selectedSubjects.length > 0 || selectedCredits.length > 0 || selectedClassTypes.length > 0;
  };
  ...
  const handleAddClass = async (classItem) => {
    ...
    if (newTotalCredits > MAX_CREDIT_HOURS) {
      setErrorMessage(`Cannot add this class. Adding ${classItem.courseCode} (${classCredits} credits) would exceed the maximum of ${MAX_CREDIT_HOURS} credit hours. You currently have ${totalCredits} credits.`);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="font-semibold">Current Credit Hours: {totalCredits} / {MAX_CREDIT_HOURS}</p>
            <div className="w-full bg-gray-200 rounded-full h-2.5 mt-2">
              <div 
                className={`h-2.5 rounded-full ${totalCredits > MAX_CREDIT_HOURS ? 'bg-red-600' : 'bg-blue-600'}`} 
                style={{ width: `${Math.min(100, (totalCredits / MAX_CREDIT_HOURS) * 100)}%` }}
              ></div>
            </div>
          </div>
            ...
            <div className="mb-6 p-4 border rounded-lg bg-gray-50">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {classTypes.length > 0 ? (
                      ...
                      <p className="text-sm text-gray-500">Loading class types...</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {hasActiveFilters() && (
                <div className="mt-4 p-2 bg-blue-50 border border-blue-200 rounded text-sm">
                  <p>Active filters: 
                    {selectedSubjects.length > 0 && ` Subjects (${selectedSubjects.length})`}
                    {selectedCredits.length > 0 && ` Credits (${selectedCredits.length})`}
                    {selectedClassTypes.length > 0 && ` Types (${selectedClassTypes.length})`}
                  </p>
                </div>
              )}
            </div>
                    ...
                    >
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                          <span className="text-sm text-gray-600">Credits: {classItem.creditHours || "N/A"}</span>
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {isAdded ? "Added" : isAddingClass ? "Adding..." : "Add Class"}
                        </button>
                        <button
